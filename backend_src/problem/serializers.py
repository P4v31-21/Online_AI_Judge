...
class AIGenerateProblemSerializer(serializers.Serializer):
    knowledge_tags = serializers.CharField(max_length=256)
    difficulty = serializers.CharField(max_length=16)
    language = LanguageNameChoiceField(required=False, allow_blank=True, default="C")
    extra_requirements = serializers.CharField(max_length=500, required=False, allow_blank=True, default="")
    # 前端传入的 AI 模型显示名（例如 "DeepSeek V3.2" / "DeepSeek R1" / "GLM-5.1 Pro"）
    # 后端通过 MODEL_NAME_MAP 映射为真实的 API 模型 ID；留空则走默认模型
    model = serializers.CharField(max_length=64, required=False, allow_blank=True, default="")


class AIBatchGenerateProblemSerializer(serializers.Serializer):
    """批量 AI 出题请求的验证器：内层复用单题 Serializer，保证字段规则一致"""
    requests = serializers.ListField(
        child=AIGenerateProblemSerializer(),
        min_length=1,
        max_length=10  # 与前端 UI 中 "最多10道" 的限制对齐
    )


class AISaveProblemSerializer(serializers.Serializer):
    _id = serializers.CharField(max_length=32, required=False, allow_blank=True, allow_null=True)
    title = serializers.CharField(max_length=1024)
    description = serializers.CharField()
    input_description = serializers.CharField()
    output_description = serializers.CharField()
    samples = serializers.ListField(child=CreateSampleSerializer(), allow_empty=True, required=False, default=list)
    sample_input = serializers.CharField(trim_whitespace=False, required=False, allow_blank=True, default="")
    sample_output = serializers.CharField(trim_whitespace=False, required=False, allow_blank=True, default="")
    time_limit = serializers.IntegerField(min_value=1, max_value=1000 * 60)
    memory_limit = serializers.IntegerField(min_value=1, max_value=1024)
    languages = LanguageNameMultiChoiceField()
    ...
    hint = serializers.CharField(allow_blank=True, allow_null=True, required=False, default="")
    source = serializers.CharField(max_length=256, allow_blank=True, allow_null=True, required=False,
                                   default="AI Generated")
    share_submission = serializers.BooleanField(required=False, default=False)
    test_case_id = serializers.CharField(max_length=32, required=False, allow_blank=True, default="")
    test_case_score = serializers.ListField(child=CreateTestCaseScoreSerializer(), allow_empty=True,
                                            required=False, default=list)
    # AI 生成的完整测试用例数组（包括样例和隐藏用例）
    # 每项形如 {"input": "...", "output": "...", "is_sample": true/false}
    # 保存题目时后端用它生成真正的测试数据文件（写到 /data/test_case/ 目录）
