...
class AIProblemAPITest(APITestCase):
    def setUp(self):
        self.create_super_admin()
        self.generate_url = self.reverse("ai_generate_problem_api")
        self.save_url = self.reverse("ai_save_problem_api")
        self.drafts_url = self.reverse("ai_my_drafts_api")

    @patch("problem.views.admin.AIGenerateProblemAPI._call_ai_model_api")
    def test_generate_problem(self, mock_call_ai):
        mock_call_ai.return_value = """{
            \"title\": \"数组最大值\",
            \"description\": \"<p>给定n个整数，输出最大值</p>\",
            \"knowledge_tags\": \"数组,循环\",
            \"sample_input\": \"5\\n1 2 3 4 5\",
            \"sample_output\": \"5\",
            \"difficulty\": \"中等\",
            \"test_cases\": [
        ...
        resp = self.client.post(self.generate_url, data={"knowledge_tags": "数组,循环", "difficulty": "中等"})
        self.assertSuccess(resp)
        self.assertEqual(resp.data["data"]["title"], "数组最大值")

    def test_save_problem_and_get_drafts(self):
        payload = {
            "title": "AI草稿题",
        ...
        save_resp = self.client.post(self.save_url, data=payload)
        self.assertSuccess(save_resp)
        self.assertTrue(save_resp.data["data"]["_id"].startswith("AI-"))
        self.assertTrue(save_resp.data["data"]["test_case_id"])
        self.assertEqual(len(save_resp.data["data"]["test_case_score"]), 1)

        test_case_id = save_resp.data["data"]["test_case_id"]
        test_case_dir = os.path.join(settings.TEST_CASE_DIR, test_case_id)
        self.assertTrue(os.path.isdir(test_case_dir))
        self.assertTrue(os.path.isfile(os.path.join(test_case_dir, "1.in")))
        self.assertTrue(os.path.isfile(os.path.join(test_case_dir, "1.out")))
        self.assertTrue(os.path.isfile(os.path.join(test_case_dir, "info")))

        drafts_resp = self.client.get(f"{self.drafts_url}?limit=10&offset=0")
        self.assertSuccess(drafts_resp)
        self.assertGreaterEqual(drafts_resp.data["data"]["total"], 1)
