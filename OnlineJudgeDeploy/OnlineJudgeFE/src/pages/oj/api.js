...
export default {
  ...
  // 对指定提交进行 AI 评价
  getAIEvaluation(data) {
    return ajax('ai/score-submission/', 'post', { data })
  },
  // 获取某提交已有的 AI 评价（如果之前已经生成）
  getSubmissionAIEvaluation(submissionId) {
    return ajax('submission', 'get', {
      params: { id: submissionId }
    })
    // 后端应在 submission 详情中返回 ai_evaluation 字段
  }
}
  ...
  return new Promise((resolve, reject) => {
    axios({
      ...
      if (res.data.error !== null) {
        const errMsg = res.data.data
        if (res.data.err === 'guard-blocked') {
          Vue.prototype.$warning(errMsg)
        } else {
          Vue.prototype.$error(errMsg)
        }
        reject(res)
        // 若后端返回为登录，则为session失效，应退出当前登录用户
        if (typeof errMsg === 'string' && errMsg.startsWith('Please login')) {
      ...
      reject(res)
      const message = (res && res.response && res.response.data && res.response.data.data)
